...
class PrTsfRecalibEngine:
    ...
    def run_recalibration(self, model_hyperparams: Dict, plot_history=False, plot_weights=False):
        ...
        # Variables to keep track of the last trained models and their ensemble
        last_trained_ensemble_models = []
        last_trained_preds_test_e = []
        k=28
        ...
        for i_t in range(self.test_set_idxs.shape[0]):
            ...
            if i_t % k == 0:
                ...
                # Clear the Keras session to free memory and reset the state
                ...
                # List to store ensemble components predictions
                preds_test_e = []
                ensemble_models = []
                ...
                #model = regression_model(settings=settings, sample_x=rec_samples.x_test)#da cambiare
                #CODE FOR XGB MODEL, NOW WHAT IS RUN IS LIGHTGBM
                ...
                #     tree_method="hist",
                #     max_bin=hyperparameters["max_bins"],
                ...
                model = lgb.LGBMRegressor(
                    ...
                    learning_rate=hyperparameters["learning_rate"],
                    #lambda_l1 = 0.001,
                    num_leaves=2**(hyperparameters["max_depth"]),  # num_leaves is a parameter unique to LightGBM
                    verbose=-1
                )
                ...
                #print(settings['x_columns_names'])
                #print(settings['pred_horiz']   )
                x_train = self.build_model_input_from_series(x=rec_block.x_train,
                                                col_names=settings['x_columns_names'],
                                                pred_horiz=settings['pred_horiz'])
                #print(x_train.shape)
                
                x_val = self.build_model_input_from_series(x=rec_block.x_vali,
                                                col_names=settings['x_columns_names'],
                                                pred_horiz=settings['pred_horiz'])
                ...
                rec_block.y_train = rec_block.y_vali
                trained_model = MultiOutputRegressor(model).fit(x_train, rec_block.y_train)
                x_in=x_train[:,:-78]
                x_ex=x_train[:,-78:]
#USED TO PLOT VARIABLE IMPORTANCE, BUT GRAPHS WERE TO DIFFERENT BETWEEN HOURS
                # for i, estimator in enumerate(trained_model.estimators_):
                #     print(f"Feature importance for target {i + 1}")
                #     lgb.plot_importance(estimator,max_num_features=30, importance_type="gain", title="LightGBM Feature Importance (Gain)")
                #     plt.show()


                #     lgb.plot_importance(estimator, max_num_features=30, importance_type='split')
                #     plt.title(f"Feature importance for target {i + 1}")
                #     plt.show()


                # #trained_model = MultiOutputRegressor(model).fit(x_train, rec_block.y_train, eval_set=[(x_val, rec_block.y_vali)])

                # train_forecasts = trained_model.predict(x_train)
               

                # # Calculate feature importances for each target
                # feature_importances = np.zeros(x_train.shape[1])
                # for estimator in trained_model.estimators_:
                #     feature_importances += estimator.feature_importances_

                # # Average the feature importances
                # feature_importances /= len(trained_model.estimators_)

                # # Get indices of the top 30 features
                # top_30_indices = np.argsort(feature_importances)[-72:]

                # # Select the top 30 features for training and validation data
                # x_train_top_30 = x_train[:, top_30_indices]
                # #x_val_top_30 = x_val[:, top_30_indices]

                # trained_model = MultiOutputRegressor(model).fit(x_train_top_30, rec_block.y_train)  



                    # Store the trained model and its prediction on test sample
                ensemble_models= trained_model
                x_test = self.build_model_input_from_series(x=rec_samples.x_test,
                                                col_names=settings['x_columns_names'],
                                                pred_horiz=settings['pred_horiz'])
                
                #preds_test_e = trained_model.predict(x_test)
                preds_test_e = trained_model.predict(x_test)


                # Store the trained ensemble models and predictions
                last_trained_ensemble_models = ensemble_models
                last_trained_preds_test_e = preds_test_e
            else:
                #print('Using the last trained ensemble models for predictions')
                # Use the last trained ensemble models for predictions
                preds_test_e = []
                x_test = self.build_model_input_from_series(x=rec_samples.x_test,
                                                col_names=settings['x_columns_names'],
                                                pred_horiz=settings['pred_horiz'])
                preds_test_e = trained_model.predict(x_test)#
                #preds_test_e = trained_model.predict(x_test)#da cambiare il predict

            # Aggregate ensemble predictions
            #ensem_preds_test = ensemble.aggregate_preds(preds_test_e)
            ensem_preds_test = preds_test_e

            # Build and store the prediction quantiles for the current test samples using the selected method
            ens_p = self.get_preds_test_quantiles(preds_test=ensem_preds_test)
            # #print(ens_p)
            # print(ens_p.shape)
            # print(ens_p.shape[-1])
            ens_p= ens_p.reshape(ens_p.shape[1],ens_p.shape[0])
            # print(ens_p.shape)
            # print(ens_p.shape[-1])
        ...
        self.__save_results__(test_results_df)#va tenuto perchè cambia nome colonna
    ...
    def get_preds_test_quantiles(self, preds_test):
        ...
        return self.__get_qr_PIs__(preds_test=preds_test)
    ...
    @staticmethod
    def __get_qr_PIs__(preds_test):#qui c'era un settings in input che ho tolto
