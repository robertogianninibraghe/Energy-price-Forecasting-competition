...
from tools.models.models_tools import regression_model, Ensemble, get_model_class_from_conf, regression_model2
...
class PrTsfRecalibEngine:
    ...
    def __instantiate_preproc__(self):
        ...
        elif self.data_configs.preprocess == 'None':
            preproc = {
                'feat': None,
                'target': None
            }
    ...
    def __build_target_quantiles__(target_alpha: List):
        ...
        """
        #target_quantiles = []
        print("No quantile 0.5 in target quantiles")
    ...
    def run_recalibration(self, model_hyperparams: Dict, plot_history=False, plot_weights=False):
        ...
        #K is the parameter for choosing how many times I want to recalibrate the model
        k=28
        ...
        for i_t in range(self.test_set_idxs.shape[0]):
            ...
            if i_t % k == 0:
                ...
                for e in range(settings['num_ense']):
                    # Clear the Keras session to free memory and reset the state
                    tf.keras.backend.clear_session()
                    model = regression_model(settings=settings, sample_x=rec_samples.x_test)
                    ...
                    #IF i am ensambling with arx model
                    if settings['model_class'] == 'RES-ARX':
                        print('Retraining ensemble models with ARX')
                        model2 = regression_model2(settings=settings, sample_x=rec_samples.x_test)
                        model2.fit(train_x=rec_block.x_train, train_y=rec_block.y_train,
                                val_x=rec_block.x_vali, val_y=rec_block.y_vali,
                                plot_history=plot_history
                                )
                        # Store the trained model and its prediction on test sample
                        ensemble_models.append(model2)
                        preds_test_e.append(model2.predict(rec_samples.x_test))
                ...
                preds_test_e = []
                for model in last_trained_ensemble_models:
                    preds_test_e.append(model.predict(rec_samples.x_test))
